package com.youmai.project.http;

/**
 * Created by Administrator on 2018/5/17 0017.
 */

public class HandlerConstant {

    public static final int REQUST_ERROR=1110;
    public static final int SEND_CODE_SUCCESS=1111;
    public static final int GET_GOODS_TYPE_SUCCESS=1112;
    public static final int LOGIN_SUCCESS=1113;
    public static final int GET_LOCATION_GOODS_SUCCESS=1114;
    public static final int GET_ACCESS_TOKEN_SUCCESS=1115;
    public static final int GET_MYGOODS_SUCCESS=1116;
    public static final int ADD_GOODS_SUCCESS=1117;
    public static final int CERIFICATION_SUCCESS=1118;
    public static final int SET_USERPIC_SUCCESS=1119;
    public static final int SET_NICKNAME_SUCCESS=1120;
    public static final int GET_USERINFO_SUCCESS=1121;
    public static final int RECHARGE_SUCCESS=1122;
    public static final int BUY_SUCCESS=1123;
    public static final int GET_VERSION_SUCCESS=1124;
    public static final int DOWNLOAD_SUCCESS=1125;
    public static final int DOWNLOAD_PRORESS=1126;
    public static final int GET_PAY_ORDER_SUCCESS=1127;
    public static final int SEARCH_BOODS_SUCCESS=1128;
    public static final int SET_ORDER_COMPLETE_SUCCESS=1129;
    public static final int SET_ORDER_CANCLE_SUCCESS=1130;
    public static final int GET_NEAR_STORE_SUCCESS=1131;
    public static final int GET_GOOD_BYID_SUCCESS=1132;
    public static final int GET_LOCATION_GOODS_SUCCESS2=1133;
    public static final int GET_PAY_ORDER_SUCCESS2=1134;
    public static final int GET_MYGOODS_SUCCESS2=1135;
    public static final int DELETE_BABY_SUCCESS=1136;
    public static final int GET_GOODS_BY_STOREID_SUCCESS=1137;
    public static final int SEARCH_BOODS_SUCCESS2=1138;
    public static final int GET_GOODS_BY_STOREID_SUCCESS2=1139;
    public static final int GET_M_ORDER_SUCCESS=1140;
    public static final int GET_M_ORDER_SUCCESS2=1141;
    public static final int STORE_EVALUATE_SUCCESS=1142;
    public static final int ADD_COMMENT_SUCCESS=1143;
    public static final int GET_COMMENT_LIST_SUCCESS=1144;
    public static final int GET_COMMENT_LIST_SUCCESS2=1145;
    public static final int GET_ORDER_DETAILS_SUCCESS=1146;
    public static final int WITHDRAWAL_SUCCESS=1147;
    public static final int GET_LOCATION_COUNT_SUCCESS=1148;
    public static final int GET_GOODS_DETAILS_SUCCESS=1149;
    public static final int GET_REPORT_LIST_SUCCESS=1150;
    public static final int REPORT_GOODS_SUCCESS=1151;
    public static final int GET_STORE_INFO_SUCCESS=1152;
    public static final int GET_STORE_INFO_SUCCESS2=1153;
    public static final int DELETE_ORDER_SUCCESS=1154;
    public static final int REPLY_SUCCESS=1155;
    public static final int ADD_ADDRESS_SUCCESS=1156;
    public static final int GET_ADDRESS_LIST_SUCCESS=1157;
}
